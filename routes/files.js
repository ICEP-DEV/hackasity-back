...
router.post('/hacker/login', (req, res) => {
    ...
    mysqlConnection.query('SELECT * FROM hacker where stu_email = ?', [email],(error, results)=>{
     if(error){
         console.log(error)
     }
      if(results.length > 0){
        bcrypt.compare(password, results[0].stu_password, (error, response) =>{
            if(error){
                res.json({
                    success: false,
                    message: "error compering"
...
router.get('/files', (req, res)=> {
    ...
    router.post('/team/points', (req,res) => {
        ...
        mysqlConnection.query("insert into team set ?",{group_name: group_name,points: points},(error, results) =>{
            if(error){
    ...
    router.get('/team/points', (req, res)=> {
        mysqlConnection.query('SELECT * FROM team ORDER BY group_id  desc',(error, results) =>{
            if(error){
                console.log(error)
            }else{
                res.json({
    ...
    router.get('/publishing',(req,res)=>{
        mysqlConnection.query('SELECT group_name,sum(points) as total from team group by group_name HAVING sum(points) ORDER BY total DESC',(err,results,fields)=>{
            if(!err)
            res.json({
                success: true, results, 
            }) 
